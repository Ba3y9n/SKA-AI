...
- حالة أحدث صورة للمستخدم: ${userContext.galleryStatus || 'لا توجد صورة مرفوعة بعد'}`;
      ...
      console.warn('Gemini generation caught:', apiErr);
    }
  }

  // Intelligent fallback for all queries
  const lower = message.trim().toLowerCase();
  if (lower.includes('السلام') || lower.includes('سلام')) {
    return 'وعليكم السلام ورحمة الله وبركاته، أهلاً بك! أنا رِواء المساعد الذكي لكلية الأعمال والاقتصاد. وش حابة تستفسرين عنه اليوم؟';
  } else if (lower.includes('كيف حالك') || lower.includes('شخبارك') || lower.includes('علومك') || lower.includes('أهلا') || lower.includes('مرحبا') || lower.includes('هلا')) {
    return 'هلا والله، أهلاً بك! أنا بخير ومبتهجة باحتفالات اليوم الوطني 96 بكلية الأعمال والاقتصاد. تفضلي بالسؤال عن الكلية أو أقسام المنصة.';
  } else if (lower.includes('أشارك') || lower.includes('أضيف') || lower.includes('اشارك') || lower.includes('اضيف')) {
    return 'تقدرين تشاركين بصورتك في معرض اللحظات، أو تسجلين طموحك للوطن في جدار المستقبل، أو توثقين إنجازاتك وجوائزك في قسم الإنجازات.';
  } else if (lower.includes('أقسام') || lower.includes('اقسام') || lower.includes('المنصة')) {
    return 'منصة رِواء تحتوي على: معرض صور احتفالات الكلية في البهو، جدار طموحات المستقبل، وسجل إنجازات طالبات وأعضاء هيئة التدريس بكلية الأعمال والاقتصاد.';
  } else if (lower.includes('هوية') || lower.includes('عزنا') || lower.includes('شعار')) {
    return 'هوية اليوم الوطني 96 هي «عزّنا بطبعنا»، وتعبر عن أصالة وعراقة وقيم المجتمع السعودي ونمائه المستمر.';
  } else if (lower.includes('صورتي') || lower.includes('مشاركتي')) {
    return 'تقدرين تتابعين حالة صورتك المرفوعة مباشرة من زر «مشاركاتي» في قسم المعرض أسفل الصفحة.';
  }

  return 'أهلاً بك! يسعدني الحديث معك ومساعدتك في التعرف على فعاليات كلية الأعمال والاقتصاد وأقسام منصة اليوم الوطني 96.';
}

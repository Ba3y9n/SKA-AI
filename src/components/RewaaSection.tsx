...
export const RewaaSection: React.FC<RewaaSectionProps> = ({ onTalk, isListening }) => {
  return (
    <section className="relative w-full min-h-screen py-24 bg-[#F8FBF8] flex flex-col items-center justify-center overflow-hidden z-20">
      ...
      >
        <h2 className="text-4xl md:text-5xl font-black text-[#064C3B] mb-4">
          أهلًا بك، أنا رِواء.
        </h2>
        <p className="text-xl md:text-2xl text-[#008F68] font-bold mb-6">
        ...
        <button 
          ...
          className={`group relative inline-flex items-center gap-4 px-10 py-5 rounded-full font-black text-lg transition-all duration-300 ${
            isListening 
              ...
              : 'bg-[#008F68] text-white hover:bg-[#006C4F] shadow-lg hover:shadow-xl hover:-translate-y-1'
          }`}
        >
          <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isListening ? 'bg-red-500 text-white animate-pulse' : 'bg-white text-[#008F68]'}`}>
